...
GaussianList::GaussianList(int _npeaks):PeakList(_npeaks), fwhm(0.5)
{
}
...
AtomIsoAbun::AtomIsoAbun():niso(0), z(0), isotopes(NULL)
{
   symbol[0] = '\0';
}

AtomIsoAbun::AtomIsoAbun(int _z, int _niso):niso(_niso), z(_z)
{
...
MolComposition::MolComposition():totalnat(0), atom(NULL), nat(NULL), z(0)
{
}
...
IsoCalc::IsoCalc():thr(0.001), degen(0.1), atomsloaded(false), molloaded(false), normalized(false), calculated(false), autocalc(false)
{
}
   ...
   {                            //do a second parse on the formula to tally up the number of each type of atom
      ...
      {                         // if current character is a capital letter - then it is the start of a new element
         ...
         {                      //if it is not the very first character, add on the appropriate number of atoms
            c = 0;
            while (c < molecule.totalnat && strcmp(molecule.atom[c].symbol, currelement.c_str()))
            {
               c++;
            }                   //look up where to add to
            if (c >= molecule.totalnat)
               return 2;        //element not found - formula is inconsistent
