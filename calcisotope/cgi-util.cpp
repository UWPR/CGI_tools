...
const char  *cgi_error_strings[CGIERR_NUM_ERRS] = {
...
char  *read_POST()
{
   ...
   int    query_size = atoi(cl_env);
   if (query_size <= 0 || query_size > 10 * 1024 * 1024)
      return NULL;
   ...
   {
      if (fread(query_string, query_size, 1, stdin) < 1)
         query_string[0] = '\0';
      else
...
void unescape_url(char *url)
{
   ...
   {
      if ((url[x] = url[y]) == '%' && y < len - 2)      /* 2.0.4 - MJ Pomraning (pilcrow@mailbag.com) */
      {
         url[x] = x2c(&url[y + 1]);
         y += 2;
      }
...
int lineread(FILE * stream,
   char *buf,
   int count)
{
   if (!fgets(buf, count, stream))
      buf[0] = '\0';
   ...
   else
   {
      ...
      if (strcmp(getenv("REQUEST_METHOD"), "POST") == 0)
      {
                  ...
                  getenv("CONTENT_TYPE"))
         {
            ...
            if (!fgets(cgi_query, cl + 1, stdin))
               cgi_query[0] = '\0';


            /* Verify that we got as much data as we expected: */

            if (strlen(cgi_query) != (size_t)cl)
                         ...
                         "multipart/form-data") == getenv("CONTENT_TYPE"))
         {
            ...
            boundary = strdup(strstr(getenv("CONTENT_TYPE"), "boundary=") + 9);
                  ...
                  else          /* (Not a boundary) */
                  {
                        ...
                        else    /* (Not a blank line) */
                        {
                                      ...
                                      "Content-Disposition: ") == cgi_query)
                           {
                              ...
                              if (strstr(cgi_query, "name=\"") != NULL)
                              {
                                 ...
                                 cgi_entries = (cgi_entry_type *)realloc(cgi_entries, sizeof(cgi_entry_type) * cgi_num_entries);
                                 ...
                                 cgi_entries[which_entry].name = strdup(strstr(cgi_query, "name=\"") + 6);


                                 /* Truncate after quote: */

                                 if (strchr(cgi_entries[which_entry].name, '\"') != NULL)
                                 {
                                    strcpy(strchr(cgi_entries[which_entry].name, '\"'), "\0");
                                 }


                                 /* Set default content-type: */

                                 cgi_entries[which_entry].content_type = "application/octet-stream";


                                 /* Set default content-length: */

                                 cgi_entries[which_entry].content_length = 0;


                                 /* Set default value: */

                                 cgi_entries[which_entry].val = strdup("");


                                 debug("entry.name", cgi_entries[which_entry].name);
                              }
                           }
                           else if (strstr(cgi_query, "Content-Type: ") == cgi_query)
                           {
                              /* Content-type: */

                              cgi_entries[which_entry].content_type =
                                 strdup(strstr(cgi_query, "Content-Type: ") + 14);
                     ...
                     else       /* in_multipart_headers == 0 */
                     {
                        ...
                        if (which_entry != -1)
                        {
                           ...
                           cgi_entries[which_entry].val = (char *)realloc(cgi_entries[which_entry].val, strlen(cgi_entries[which_entry].
                                             val) + length_gotten + 1);
                           ...
                           memcpy(cgi_entries[which_entry].val +
                                  (cgi_entries[which_entry].
                                   content_length), cgi_query, length_gotten);

                           cgi_entries[which_entry].content_length =
                              (cgi_entries[which_entry].content_length +
                               length_gotten);

                           cgi_entries[which_entry].val[cgi_entries[which_entry].content_length]=0;
                        }
                     }
                  }
               }
      ...
      if (cgi_content_type != CGITYPE_MULTIPART_FORM_DATA)
      {
         ...
         cgi_entries = (cgi_entry_type *) malloc(sizeof(cgi_entry_type) * cgi_num_entries);
         if (cgi_entries == NULL)
         {
            cgi_errno = CGIERR_OUT_OF_MEMORY;
            return (cgi_errno);
      ...
      for (i = 0; i < cgi_num_entries; i++)
      {
         if (cgi_entries[i].name == NULL)
            cgi_entries[i].name = (char*)"";
         if (cgi_entries[i].val == NULL)
            cgi_entries[i].val  = (char*)"";
//printf("<p>%d/%d   %s = %s\n", i, cgi_num_entries, cgi_entries[i].name, cgi_entries[i].val);
      }
   }
...
const char *cgi_getcookie(const char *cookie_name)
{
   ...
   tmpcookie = (char *)malloc(sizeof(char) * (strlen(rawcookie) + 1));
   if (tmpcookie == NULL)
   {
      cgi_errno = CGIERR_OUT_OF_MEMORY;
      return (NULL);
   ...
   do
   {
      ...
      *strchr(left, '=') = '\0';
      ...
      if (strcmp(left, cookie_name) == 0)
      {
         ...
         cookieval = (char *)malloc(sizeof(char) * (strlen(right) + 1));
         if (cookieval == NULL)
         {
            cgi_errno = CGIERR_OUT_OF_MEMORY;
            return (NULL);
...
const char *cgi_getnentrystr(const char *field_name,
   int n)
{
   int    x;
   int    cnt;
...
const char *cgi_getnentrytype(const char *field_name,
   int n)
{
   int    x;
   int    cnt;
...
int cgi_getnentryint(const char *field_name,
   int n)
{
   int    v;
...
double cgi_getnentrydouble(const char *field_name,
   int n)
{
   double v;
...
int cgi_getentrybool(const char *field_name,
   int def)
{
...
int cgi_getnentrybool(const char *field_name,
   int def,
   int n)
{
   const char *temp;
   ...
   if (temp != NULL)
   {
      if (strcasecmp(temp, "yes") == 0 ||
          strcasecmp(temp, "on") == 0 || strcasecmp(temp, "true") == 0)
      {
         /* A "yes" or "on" is a 1: */
         ...
         v = 1;
               ...
               strcasecmp(temp, "off") == 0 || strcasecmp(temp, "false") == 0)
      {
         /* A "no" or "off" is a 0: */
...
int cgi_goodemailaddress(const char *addr)
{
   ...
   if (addr[strlen(addr) - 1] == '@' ||
       addr[strlen(addr) - 1] == '.' || addr[0] == '@' || addr[0] == '.')
      return 0;
   ...
   for (i = 0; (size_t)i < strlen(addr); i++)
   {
